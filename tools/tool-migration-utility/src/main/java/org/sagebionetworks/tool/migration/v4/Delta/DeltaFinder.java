		...
		if (typeToMigrateMeta.getSrcMinId() == null) {
			...
			if (typeToMigrateMeta.getDestMinId() == null) {
				...
				if (updatesMinId > updatesMaxId) { // Disjoint
					...
					// Update ranges
					try {
						updRanges.addAll(findUpdDeltaRanges(sourceClient, destinationClient, typeToMigrateMeta.getType(), updatesMinId, updatesMaxId, batchSize));
	...
	private List<IdRange> findUpdDeltaRanges(SynapseAdminClient srcClient, SynapseAdminClient destClient, MigrationType type, long minId, long maxId, long batchSize) throws SynapseException, JSONObjectAdapterException {
		...
		if (srcCrc32.getChecksum().equals(destCrc32.getChecksum())) {
			...
			if (maxId - minId < batchSize) {
				...
				long maxId2 = maxId;
				l.addAll(findUpdDeltaRanges(srcClient, destClient, type, minId1, maxId1, batchSize));
				l.addAll(findUpdDeltaRanges(srcClient, destClient, type, minId2, maxId2, batchSize));
				return l;
			}
		}
	}
