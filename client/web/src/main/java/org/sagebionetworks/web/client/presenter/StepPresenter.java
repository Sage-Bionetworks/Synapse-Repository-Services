...
public class StepPresenter extends AbstractActivity implements StepView.Presenter {
	...
	@Inject
	public StepPresenter(StepView view, NodeServiceAsync service, NodeModelCreator nodeModelCreator, AuthenticationController authenticationController, GlobalApplicationState globalApplicationState){
		this.view = view;
	...
	@Override
	public void delete() {
		nodeService.deleteNode(NodeType.STEP, stepId, new AsyncCallback<Void>() {
			@Override
			public void onSuccess(Void result) {
				view.showInfo("Step Deleted", "The step was successfully deleted.");
				placeChanger.goTo(new StepsHome(DisplayUtils.DEFAULT_PLACE_TOKEN));
			}
	...
	protected void loadFromServer() {
		...
		nodeService.getNodeJSON(NodeType.STEP, this.stepId, new AsyncCallback<String>() {
			...
			public void onFailure(Throwable caught) {
				setStep(null);
				view.showErrorMessage("An error retrieving the Step occured. Please try reloading the page.");				
			}
		});
	}
	...
	protected void setStep(final Step step) {
		if (step != null) {			
			...
			if(currentUser != null) {
				AclUtils.getHighestPermissionLevel(NodeType.STEP, step.getId(), nodeService, new AsyncCallback<PermissionLevel>() {
					@Override
	...
	private void setStepDetails(final Step step,
			boolean isAdministrator, boolean canEdit) {
		view.setStepDetails(step.getId(), step.getName(),
				step.getCommandLine(), step.getCreatedBy(),
				step.getCreationDate(), step.getStatus(), isAdministrator, canEdit);
	}	
