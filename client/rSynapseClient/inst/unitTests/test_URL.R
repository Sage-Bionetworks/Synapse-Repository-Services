...
unitTestGetHostWithPort <- function() {
	url <- .ParsedUrl('http://localhost:8080/services-authentication-0.5.2/auth/v1')
	checkEquals(url@authority, 'localhost:8080')
	checkEquals(url@host, 'localhost')
	checkEquals(url@port, '8080')
	checkEquals(url@path, '/services-authentication-0.5.2/auth/v1')
	checkEquals(url@file, 'v1')
	checkEquals(url@pathPrefix, '/services-authentication-0.5.2/auth')
}
