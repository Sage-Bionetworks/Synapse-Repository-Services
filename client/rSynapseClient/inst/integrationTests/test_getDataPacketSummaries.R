...
integrationTestForgotToLogin <- function() {
  ...
  checkException(getDataPacketSummaries(anonymous=F))
}

integrationTestAnonymousAccess <- function() {
  packets <- getDataPacketSummaries(anonymous=T)
...
integrationTestQueryForDataset <- function() {
  packets <- getDataPacketSummaries(list(where='dataset.name == "MSKCC Prostate Cancer"'), anonymous=T)
