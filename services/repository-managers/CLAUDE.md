...
- **Query paths read the bound schema; build paths re-derive it.** A read path loads the schema with `tableManagerSupport.getTableSchema(entityId)` so it never pays the cost of `QueryTranslator.build`. A build path must instead derive the schema from its own translation of the `definingSql`, because the bound schema only advances on create/update of *this* entity while a rebuild is triggered by a *source* change — at build time it may describe a shape the source no longer has. `MaterializedViewManagerImpl` rebinds after its index swap; a SearchIndex instead serves the schema from the as-built snapshot in each OpenSearch slot's `_meta` (see `manager/search/CLAUDE.md`).
