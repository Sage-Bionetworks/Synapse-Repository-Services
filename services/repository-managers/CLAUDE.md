...
- **Query paths read the bound schema; build paths re-derive it.** A read path loads the schema with `tableManagerSupport.getTableSchema(entityId)` so it never pays the cost of `QueryTranslator.build`. A build path must instead derive the schema from its own translation of the `definingSql` and rebind it once the new artifact is live (reference: `MaterializedViewManagerImpl` around its index swap, `SearchIndexLifecycleManagerImpl.buildIndex`). The bound schema only advances on create/update of *this* entity, but a rebuild is triggered by a *source* becoming AVAILABLE — so at build time the bound schema may describe a shape the source no longer has, and building against it pairs stale column types and ordering with freshly translated row values (evidence: PLFM-9714).
