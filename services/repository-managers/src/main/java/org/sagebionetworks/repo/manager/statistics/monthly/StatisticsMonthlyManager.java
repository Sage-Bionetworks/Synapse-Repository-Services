...
public interface StatisticsMonthlyManager {
	...
	/**
	 * Returns the list of {@link YearMonth} that might need to be processed for the given type of
	 * object, in particular all the months that do not have any status or that are not in
	 * {@link StatisticsStatus#AVAILABLE} status.
	 * 
	...
	/**
	 * Set the status for the object of the given type and the given month to PROCESSING if the status
	 * is not already available or if the given month is in processing status and the lastStartedOn
	 * exceeded the allowed timeout. If the status was updated sends a notification to the background
	 * workers to start processing.
	 * 
	 * @param objectType
	 * @param month
	...
	/**
	 * Process the statistics for the given object type and month. If any error occurs during the
	 * processing sets the status to {@link StatisticsStatus#PROCESSING_FAILED}, otherwise set it to
	 * {@link StatisticsStatus#AVAILABLE} at the end of the processing. This method is invoked by a
	 * worker upon receiving a notification to start processing a particular month for a given object
	 * type. A listener will be registered on the given {@link ProgressCallback} to update the
