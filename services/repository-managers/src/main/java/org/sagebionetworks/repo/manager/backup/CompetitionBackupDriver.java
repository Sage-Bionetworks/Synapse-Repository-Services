...
public class CompetitionBackupDriver implements GenericBackupDriver {
	...
	public boolean writeBackup(File destination, Progress progress,
		...
		try {
			...
			for (String idToBackup : competitionIdsToBackup) {
				...
				Competition comp = competitionDAO.get(idToBackup);
				ZipEntry entry = new ZipEntry(idToBackup + ZIP_ENTRY_SUFFIX);
				zos.putNextEntry(entry);
				NodeSerializerUtil.writeCompetitionBackup(comp, zos);
	...
	public boolean restoreFromBackup(File source, Progress progress)
		...
		try {
			...
			while ((entry = zin.getNextEntry()) != null) {
				...
				Competition backup = NodeSerializerUtil
						.readCompetitionBackup(zin);
				createOrUpdateCompetition(backup);
	...
	private void createOrUpdateCompetition(Competition comp)
			throws DatastoreException, NotFoundException,
			InvalidModelException, ConflictingUpdateException {
		// create the activity
		Competition existingComp = null;
		try {
			existingComp = competitionDAO.get(comp.getId().toString());
		} catch (NotFoundException e) {
			existingComp = null;
		}
		if (null == existingComp) {
			// create
			competitionDAO.create(comp, comp.getOwnerId());
		} else {
			// Update only when backup is different from the current system
			if (!existingComp.getEtag().equals(comp.getEtag())) {
				competitionDAO.updateFromBackup(comp);
			}
		}
	}
