...
`query_string` names its columns inside the Lucene `query` text, so `SearchFieldRewriter.rewriteQueryStringClause` tokenizes that expression and substitutes column ids for its field prefixes, and is consequently where the clause's unknown-column rejection lives.
The tokenizer is generated by JavaCC from `src/main/javacc/QueryStringLexer.jj`, whose token definitions are a verbatim copy of Lucene's classic `QueryParser.jj` for the Lucene version the OpenSearch domain runs; when the domain's engine version changes, re-diff the copy against that Lucene release so the lexer keeps splitting expressions exactly as OpenSearch does.
