...
public class MembershipInvitationManagerImpl implements
	...
	public MembershipInvtnSubmission create(UserInfo userInfo,
		...
		if (!authorizationManager.canAccess(userInfo, mis.getTeamId(), ObjectType.TEAM, ACCESS_TYPE.TRAM_MEMBERSHIP_UPDATE)) throw new UnauthorizedException("Cannot create membership invitation.");
		Date now = new Date();
	...
	public MembershipInvtnSubmission get(UserInfo userInfo, String id)
		...
		if (!authorizationManager.canAccess(userInfo, mis.getTeamId(), ObjectType.TEAM, ACCESS_TYPE.TRAM_MEMBERSHIP_UPDATE)) throw new UnauthorizedException("Cannot retrieve membership invitation.");
		return mis;
	}
	...
	public void delete(UserInfo userInfo, String id) throws DatastoreException,
		...
		MembershipInvtnSubmission mis = membershipInvtnSubmissionDAO.get(id);
		if (!authorizationManager.canAccess(userInfo, mis.getTeamId(), ObjectType.TEAM, ACCESS_TYPE.TRAM_MEMBERSHIP_UPDATE)) throw new UnauthorizedException("Cannot delete membership invitation.");
