...
public class ParticipantManagerTest {
    ...
    public void setUp() throws DatastoreException, NotFoundException, InvalidModelException {
    	...
    	when(mockUserGroup.getId()).thenReturn("786");
    	when(mockUserInfo.getIndividualGroup()).thenReturn(mockUserGroup);
    	when(mockUserManager.getUserInfo(USER_ID)).thenReturn(mockUserInfo);
    	mockUserInfo = mock(UserInfo.class);
    	mockUserGroup = mock(UserGroup.class);
    	when(mockUserGroup.getId()).thenReturn("456");
    	when(mockUserInfo.getIndividualGroup()).thenReturn(mockUserGroup);
    	when(mockUserManager.getUserInfo(OWNER_ID)).thenReturn(mockUserInfo);
