...
public class MembershipInvitationManagerImplTest {
	...
	public void testNonAdminCreate() throws Exception {
		...
		when(mockAuthorizationManager.canAccess(userInfo, mis.getTeamId(), ObjectType.TEAM, ACCESS_TYPE.MEMBERSHIP)).thenReturn(false);
	...
	public void testAdminCreate() throws Exception {
		MembershipInvtnSubmission mis = createMembershipInvtnSubmission(null);
		when(mockAuthorizationManager.canAccess(userInfo, mis.getTeamId(), ObjectType.TEAM, ACCESS_TYPE.MEMBERSHIP)).thenReturn(true);
		membershipInvitationManagerImpl.create(userInfo, mis);
	...
	public void testGetOpenForUserInRange() throws Exception {
		...
		QueryResults<MembershipInvitation> actual = membershipInvitationManagerImpl.getOpenForUserInRange(MEMBER_PRINCIPAL_ID,0,1);
	...
	public void testGetOpenForUserAndTeamInRange() throws Exception {
		...
		QueryResults<MembershipInvitation> actual = membershipInvitationManagerImpl.getOpenForUserAndTeamInRange(MEMBER_PRINCIPAL_ID, TEAM_ID, 0,1);
		assertEquals(expected, actual.getResults());
		assertEquals(1L, actual.getTotalNumberOfResults());
	}
