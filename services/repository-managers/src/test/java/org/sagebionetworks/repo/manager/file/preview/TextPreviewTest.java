...
public class TextPreviewTest {
	...
	public void testGeneratePreview() throws IOException {
		...
		//output should contain the entire last line (since it's short enough)
		assertTrue(output.indexOf(lastLine) > -1);
	}
	@Test
	public void testGeneratePreviewLineLimit() throws IOException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		StringBuilder inputString = new StringBuilder();
		//add more lines than the max
		for (int i = 0; i < TextPreviewGenerator.MAX_LINE_COUNT + 100; i++) {
			inputString.append("This is line " + i + "\n");
		}
		
		InputStream from = IOUtils.toInputStream(inputString.toString());
		PreviewOutputMetadata type = textPreviewGenerator.generatePreview(from, baos);
		assertEquals(TextPreviewGenerator.TEXT_PLAIN, type.getContentType());
		String output = baos.toString();
		String[] outputLines = output.split("\n");
		//verify only MAX_LINE_COUNT are output
		assertEquals(TextPreviewGenerator.MAX_LINE_COUNT, outputLines.length);
	}
