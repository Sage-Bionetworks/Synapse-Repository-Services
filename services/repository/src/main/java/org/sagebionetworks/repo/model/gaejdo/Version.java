...
/**
 * A class to implement versions, like "1.20.3" or "A.10", which implements Comparable
 * 
 * @author bhoff
 *
 */
public class Version implements Comparable<Version>, Serializable {
	...
	/**
	 * 
	 * @param s a string of values separated by '.', e.g. "1.20.3" or "A.10"
	 */
	...
	public int compareTo(Version that) {
		...
		if (thisL.size()<thatL.size()) return -1;
		if (thisL.size()>thatL.size()) return 1;
		return 0;
	}
	...
	public String toString() {
		...
		for (Comparable c : getL()) {
			if (first) first=false; else sb.append(getSeparator());
			sb.append(c);
		}
		return sb.toString();
	}
	...
	/**
	 * Increments the version.  Note: This method may only be used in the case that
	 * the values in the Version's fields are Integer.
	...
	public Version increment(int field) {
		...
		if (!(o instanceof Integer)) 
			throw new ClassCastException("Can't increment object of type "+o.getClass());
