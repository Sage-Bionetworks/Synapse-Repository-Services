...
public class MembershipInvitationController {
	...
	public @ResponseBody
	MembershipInvitation getInvitationById(
			@PathVariable String id,
			@RequestParam(value = AuthorizationConstants.USER_ID_PARAM) Long userId
			) throws NotFoundException {
	...
	public @ResponseBody
	MembershipInvitation getInvitationByToken(
			@PathVariable String id,
	 ...
	 * @param id the membership invitation ID
	 * @param userId
	 * @return
	 * @throws NotFoundException
	 */
	...
	public @ResponseBody InviteeVerificationSignedToken getInviteeVerificationSignedToken(
		...
		return serviceProvider.getMembershipInvitationService().getInviteeVerificationSignedToken(userId, id);
	}
	 ...
	 * See https://sagebionetworks.jira.com/wiki/spaces/PLFM/pages/143628166/Invite+a+new+user+to+join+a+team for more information.
	 * @param id the MembershipInvitation ID
	 * @param userId
	 * @param token
	 */
	@RequiredScope({modify})
	...
	public void updateInviteeId(
		...
		serviceProvider.getMembershipInvitationService().updateInviteeId(userId, id, token);
	}
}
