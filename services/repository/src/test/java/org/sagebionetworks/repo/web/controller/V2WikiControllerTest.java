...
public class V2WikiControllerTest {
	...
	private void doWikiCRUDForOwnerObject(String ownerId, ObjectType ownerType) throws Exception{
		...
		URL presigned  = entityServletHelper.getV2WikiAttachmentFileURL(adminUserId, childKey, handleOne.getFileName(), null);
		...
		presigned  = entityServletHelper.getV2WikiAttachmentPreviewFileURL(adminUserId, childKey, handleOne.getFileName(), null);
		assertNotNull(presigned);
		...
		presigned  = entityServletHelper.getV2WikiAttachmentFileURL(adminUserId, childKey, handleOne.getFileName(), redirect);
		assertNotNull(presigned);
		assertTrue(presigned.toString().indexOf("mainFileKey") > 0);
		System.out.println(presigned);
		// again without the redirct
		presigned  = entityServletHelper.getV2WikiAttachmentPreviewFileURL(adminUserId, childKey, handleOne.getFileName(), redirect);
		assertNotNull(presigned);
		assertTrue(presigned.toString().indexOf("previewFileKey") > 0);
		System.out.println(presigned);
