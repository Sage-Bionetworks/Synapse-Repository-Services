...
public class MessageControllerTest {
	...
	public void testCreateMessage() throws Exception {
		...
		request.setContent("{\"text\":\"message from a unit test\"}"
				.getBytes("UTF-8"));
		servlet.service(request, response);
		log.info("Results: " + response.getContentAsString());
		...
		// {"id":1,"text":"message from a unit test"}
	...
	public void testInvalidModelCreateMessage() throws Exception {
		...
		request.setRequestURI("/message");
		request.addHeader("Content-Type", "application/json; charset=UTF-8");
		request.setContent("{\"textBROKEN\":\"message from a unit test\"}"
				.getBytes("UTF-8"));
		servlet.service(request, response);
		log.info("Results: " + response.getContentAsString());
		assertEquals(HttpStatus.BAD_REQUEST.value(), response.getStatus());
		...
		// \"textBROKEN\"
		// (Class org.sagebionetworks.repo.model.Message), not marked as
		// ignorable\n at
	...
	/**
	 * Test method for
	 * {@link org.sagebionetworks.repo.web.controller.MessageController#updateEntity}
	 * .
	 * 
	 * @throws Exception
	 */
	...
	public void testUpdateMessage() throws Exception {
		...
		// The response should be something like:
		// {"id":1,"text":"updated message from a unit test"}
		assertTrue(0 < results.getInt("id"));
		assertEquals("updated message from a unit test",
	...
	public void testUpdateMessageMissingEtag() throws Exception {
		...
		request.addHeader("Content-Type", "application/json; charset=UTF-8");
		request.setContent("{\"id\": 1, \"text\":\"updated message from a unit test\"}"
				.getBytes("UTF-8"));
		servlet.service(request, response);
		log.info("Results: " + response.getContentAsString());
		assertEquals(HttpStatus.BAD_REQUEST.value(), response.getStatus());
		...
		// org.sagebionetworks.repo.web.controller.MessageController.updateMessage
	...
	public void testUpdateNonExistentMessage() throws Exception {
		...
		request.addHeader("Content-Type", "application/json; charset=UTF-8");
		request.setContent("{\"id\": 100, \"text\":\"updated message from a unit test\"}"
				.getBytes("UTF-8"));
		servlet.service(request, response);
		log.info("Results: " + response.getContentAsString());
		assertEquals(HttpStatus.NOT_FOUND.value(), response.getStatus());
		JSONObject results = new JSONObject(response.getContentAsString());
		// The response should be something like:
		// {"reason":"no message with id 100 exists"}
		assertNotNull(results.getString("reason"));
	}
}
