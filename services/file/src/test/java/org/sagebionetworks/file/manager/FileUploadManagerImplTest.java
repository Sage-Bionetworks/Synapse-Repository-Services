...
public class FileUploadManagerImplTest {
	...
	public void before() throws UnsupportedEncodingException, IOException{
		...
		// the manager to test.
		manager = new FileUploadManagerImpl();
	}
	...
	public void testUploadfilesWithExpectedParams() throws FileUploadException, IOException{
		...
		FileData fileData = results.getFiles().get(0);
		assertEquals("text/plain", fileData.getContentType());
		assertEquals("someTextFile.txt", fileData.getFileName());
		assertNotNull(fileData.getFileToken());
		// The file token should be:
		// userId/UUID/Filename
		assertTrue(fileData.getFileToken().startsWith("987/"));
		assertTrue(fileData.getFileToken().endsWith("/someTextFile.txt"));
	}
