...
public class TableCSVDownloadWorker implements MessageDrivenRunner {
	...
	public void run(ProgressCallback progressCallback, Message message) throws Exception {
		...
		try{
			...
			S3FileHandle fileHandle = fileHandleManager.multipartUploadLocalFile(new LocalFileUploadRequest().withFileToUpload(temp).withContentType(contentType).withListener(uploadListener));
