 ...
 * 
 */
public class TableQueryWorker implements Worker {
	...
	private Message processMessage(Message message) throws Throwable {
		...
		try{
			...
			QueryBundleRequest request = (QueryBundleRequest) status.getRequestBody();
			QueryResultBundle queryBundle = tableRowManager.queryBundle(user, request);
