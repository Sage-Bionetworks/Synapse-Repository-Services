...
public class DownloadListPackageWorkerTest {
	...
	@InjectMocks
	DownloadListPackageWorker worker;

	String jobId;
	AsynchronousJobStatus jobStatus;
	Long startedByUserId;
	UserInfo user;
	DownloadListPackageRequest requestBody;
	DownloadListPackageResponse responseBody;
	...
	public void before() {
		...
		requestBody = new DownloadListPackageRequest();
		requestBody.setZipFileName("result.zip");
		...
		responseBody = new DownloadListPackageResponse();
	...
	public void testRun() throws RecoverableMessageException, Exception {
		...
		verify(mockDownloadListManger).packageFiles(mockProgressCallback, user, requestBody);
	...
	public void testRunFailed() throws RecoverableMessageException, Exception {
		...
		when(mockDownloadListManger.packageFiles(any(), any(), any())).thenThrow(exception);
		// call under test
		worker.run(mockProgressCallback, mockMessage);
		verify(mockDownloadListManger).packageFiles(mockProgressCallback, user, requestBody);
