...
public class MessageReceiverImplTest {
	...
	public void before(){
		...
		messageReveiver = new MessageReceiverImpl(mockSQSClient, maxNumberOfWorkerThreads,visibilityTimeout, mockQueue, stubFactory);
		...
		when(mockSQSClient.receiveMessage(new ReceiveMessageRequest(queueUrl).withMaxNumberOfMessages(maxNumberOfWorkerThreads).withVisibilityTimeout(visibilityTimeout))).thenReturn(results);
	...
	public void testTrigerFiredOneFailureMulitipleSuccess() throws InterruptedException{
		...
		// This time the first message should not get delete because of the failure
