...
public class TableWorkerIntegrationTest {
	...
	public void testLimitOffset() throws Exception {
		...
		queryResult = tableQueryManger.query(mockProgressCallbackVoid, adminUserInfo, sql, null, 5L, 1L, true, false, true).getFirst();
		assertEquals(1, queryResult.getQueryResults().getRows().size());
		assertNull(queryResult.getNextPageToken());
		compareValues(rowSet, 5, 1, queryResult.getQueryResults());

		queryResult = tableQueryManger.query(mockProgressCallbackVoid, adminUserInfo, sql, null, 5L, 2L, true, false, true).getFirst();
		assertEquals(1, queryResult.getQueryResults().getRows().size());
		assertNull(queryResult.getNextPageToken());
		compareValues(rowSet, 5, 1, queryResult.getQueryResults());
		...
		queryResult = tableQueryManger.query(mockProgressCallbackVoid, adminUserInfo, sql + " limit 3", null, 0L, 100L, true, false, true).getFirst();
		assertEquals(2, queryResult.getQueryResults().getRows().size());
		assertNotNull(queryResult.getNextPageToken());
		compareValues(rowSet, 0, 2, queryResult.getQueryResults());
	...
	public void testPartialUpdateRoundTrip() throws Exception {
		...
		tableEntityManager
				.appendPartialRows(adminUserInfo, tableId, schema, partialRowSet, mockPprogressCallback);
	...
	public void testRemoveColumn() throws Exception {
		...
		tableEntityManager.appendPartialRows(adminUserInfo, tableId, schema,
	...
	public void testBooleans() throws Exception {
		...
		for (String failingBoolean : failingBooleans) {
			...
			failRowSet.setHeaders(TableModelUtils.getSelectColumns(schema, false));
			failRowSet.setTableId(tableId);
			try {
				tableEntityManager.appendRows(adminUserInfo, tableId, schema, failRowSet, mockPprogressCallback);
	...
	public void testReplaceAndDeleteRoundTrip() throws Exception {
		...
		RowReferenceSet referenceSet2 = tableEntityManager.appendRows(adminUserInfo, tableId,
				schema, rowSet, mockPprogressCallback);
	...
	public void testPermissions() throws Exception {
		...
		// Bind the columns. This is normally done at the service layer but the workers cannot depend on that layer.
		tableEntityManager.setTableSchema(adminUserInfo, Lists.transform(headers, TableModelUtils.LONG_TO_STRING), tableId);
		tableEntityManager.appendRows(owner, tableId, schema,
	...
	private QueryResult waitForConsistentQuery(UserInfo user, String sql, List<SortItem> sortItems, Long limit) throws Exception {
		...
		while(true){
			try {
				Pair<QueryResult, Long> queryResult = tableQueryManger.query(mockProgressCallbackVoid, user, sql, sortItems, 0L, limit, true, false, true);
