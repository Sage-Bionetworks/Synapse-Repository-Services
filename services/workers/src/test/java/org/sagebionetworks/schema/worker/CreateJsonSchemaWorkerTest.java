...
public class CreateJsonSchemaWorkerTest {
	...
	@InjectMocks
	CreateJsonSchemaWorker worker;
	
	String jobId;
	JsonSchema schema;
	AsynchronousJobStatus jobStatus;
	CreateSchemaRequest requestBody;
	...
	public void before() {
		...
		requestBody = new CreateSchemaRequest();
		...
		response = new CreateSchemaResponse();
		response.setNewVersionInfo(newVersionInfo);
	}
	...
	public void testRun() throws RecoverableMessageException, Exception {
		...
		when(mockUserManager.getUserInfo(any())).thenReturn(user);
		when(mockJsonSchemaManager.createJsonSchema(any(), any())).thenReturn(response);
		// call under test
		worker.run(mockProgressCallback, mockMessage);
		verify(mockAsynchJobStatusManager).lookupJobStatus(jobId);
		verify(mockUserManager).getUserInfo(startedByUserId);
		verify(mockAsynchJobStatusManager).updateJobProgress(jobId, 0L, 100L, "Starting job...");
		verify(mockJsonSchemaManager).createJsonSchema(user, requestBody);
	...
	public void testRunFailed() throws RecoverableMessageException, Exception {
		...
		when(mockAsynchJobStatusManager.lookupJobStatus(any())).thenReturn(jobStatus);
		when(mockUserManager.getUserInfo(any())).thenReturn(user);
		IllegalArgumentException exception = new IllegalArgumentException("wrong stuff");
		when(mockJsonSchemaManager.createJsonSchema(any(), any())).thenThrow(exception);
		// call under test
		worker.run(mockProgressCallback, mockMessage);
		verify(mockJsonSchemaManager).createJsonSchema(user, requestBody);
	...
	public void testRunRecoverable() throws RecoverableMessageException, Exception {
		...
		when(mockAsynchJobStatusManager.lookupJobStatus(any())).thenReturn(jobStatus);
		when(mockUserManager.getUserInfo(any())).thenReturn(user);
		RecoverableMessageException exception = new RecoverableMessageException("Can try again.");
		when(mockJsonSchemaManager.createJsonSchema(any(), any())).thenThrow(exception);
		...
		verify(mockJsonSchemaManager).createJsonSchema(user, requestBody);
		verify(mockAsynchJobStatusManager).updateJobProgress(jobId, 0L, 100L, exception.getMessage());
		verify(mockAsynchJobStatusManager, never()).setComplete(any(), any());
		verify(mockAsynchJobStatusManager, never()).setJobFailed(any(), any());
	}
}
