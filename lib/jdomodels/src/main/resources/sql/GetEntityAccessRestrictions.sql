...
WITH 
    ...
    ACC AS(
        SELECT distinct AR.REQUIREMENT_ID AS REQUIREMENT_ID, IF(RAT.STRING_ELE ='EXEMPTION_ELIGIBLE', TRUE, FALSE) AS IS_EXAMPTION_ELIGIBLE
        FROM AR
            ...
            LEFT JOIN ACL_RESOURCE_ACCESS_TYPE RAT ON (ARA.ID = RAT.ID_OID  AND RAT.STRING_ELE ='EXEMPTION_ELIGIBLE')
            WHERE ACL.ID is not null AND RAT.STRING_ELE is not null
    )
SELECT 
	EI.ENTITY_ID,
	EI.NODE_TYPE,
