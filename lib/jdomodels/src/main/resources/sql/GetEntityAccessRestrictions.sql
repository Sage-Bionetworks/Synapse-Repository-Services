...
WITH 
    ...
    ACC AS(
        SELECT EAR.REQUIREMENT_ID AS REQUIREMENT_ID, IF(RAT.STRING_ELE ='EXEMPTION_ELIGIBLE', TRUE, FALSE) AS IS_EXAMPTION_ELIGIBLE FROM EAR
            LEFT JOIN ACL ON (EAR.REQUIREMENT_ID= ACL.OWNER_ID)
            LEFT JOIN ACL_RESOURCE_ACCESS ARA ON (ACL.ID = ARA.OWNER_ID)
            LEFT JOIN ACL_RESOURCE_ACCESS_TYPE RAT ON (ARA.ID = RAT.ID_OID)
            WHERE  ARA.GROUP_ID IN (:usersGroups) AND RAT.STRING_ELE ='EXEMPTION_ELIGIBLE'
    )
SELECT 
	EI.ENTITY_ID,
	EI.NODE_TYPE,
    ...
    If(ACC.IS_EXAMPTION_ELIGIBLE, TRUE, FALSE) AS IS_EXAMPTION_ELIGIBLE
