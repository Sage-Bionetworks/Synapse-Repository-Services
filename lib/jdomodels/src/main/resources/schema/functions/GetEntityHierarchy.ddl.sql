...
BEGIN
	...
	PATH AS (
		...
		IFNULL(CONCAT(GROUP_CONCAT(PARENT_ID ORDER BY DEPTH DESC SEPARATOR ','),',') ,'') pathIds
 ...
 SELECT JSON_OBJECT('path', PATH.path, 'pathIds', CONCAT('[', PATH.pathIds, PATH.ENTITY_ID, ']'), 'projectId', PRO.PROJECT_ID, 'benefactorId', BEN.BENEFACTOR_ID) 
 	INTO resultJSON FROM PATH LEFT JOIN PRO ON (PATH.ENTITY_ID = PRO.ENTITY_ID)
