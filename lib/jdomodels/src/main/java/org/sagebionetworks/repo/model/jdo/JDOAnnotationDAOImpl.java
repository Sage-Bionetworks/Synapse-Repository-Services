 ...
 */
abstract public class JDOAnnotationDAOImpl<S extends Base, T extends JDOAnnotatable & JDOBase, A extends Comparable<A>>
	...
	abstract protected Class<? extends JDOAnnotation<A>> getAnnotationClass();

	// we need this since we can't otherwise get the type of 'A' at runtime
	abstract protected Class<A> getValueClass();

	// this is the name of the Set field in the JDOAnnotations object
	abstract protected String getCollectionName();

	abstract protected void addAnnotation(JDOAnnotations annots,
			String attr, A value);

	abstract protected void removeAnnotation(JDOAnnotations annots,
			String attr, A value);

	abstract protected Iterable<JDOAnnotation<A>> getIterable(
	...
	protected Collection<JDOAnnotations> getAnnotationsHaving(
		...
		Collection<JDOAnnotations> ans = (Collection<JDOAnnotations>) query
	...
	private Collection<A> getAnnotationValues(JDOAnnotations annots,
			String attr) {
		Collection<A> ans = new HashSet<A>();
		for (JDOAnnotation<A> annot : getIterable(annots)) {
	...
	private List<T> getSortedByAnnotation(PersistenceManager pm, String attrib,
		...
		// System.out.println("JDOAnnotationDAOImpl: ownerValueMap: "+ownerValueMap);
		// System.out.println("JDOAnnotationDAOImpl: Before sorting: "+owners);
		...
		// System.out.println("JDOAnnotationDAOImpl: After sorting: "+owners);
	...
	protected JDOAnnotation<A> getAnnotation(T jdo, String attribute, A value) {
		for (JDOAnnotation<A> annot : getIterable(jdo.getAnnotations())) {
	...
	public void addAnnotation(String id, String attribute, A value)
		...
		try {
			...
			if (hasAnnotation(jdo, attribute, value))
				return;
			JDOAnnotations annots = jdo.getAnnotations();
	...
	public void removeAnnotation(String id, String attribute, A value)
		...
		try {
			...
			Long key = KeyFactory.stringToKey(id);
			T jdo = (T) pm.getObjectById(getOwnerClass(), key);
			JDOAnnotation<A> matchingAnnot = getAnnotation(jdo, attribute, value);
