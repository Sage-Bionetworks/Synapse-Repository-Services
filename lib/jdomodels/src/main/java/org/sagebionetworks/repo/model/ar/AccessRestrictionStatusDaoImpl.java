...
public class AccessRestrictionStatusDaoImpl implements AccessRestrictionStatusDao {
	...
	public List<SubjectStatus> getEntityStatus(List<Long> entityIds, Long userId) {
		...
		namedJdbcTemplate.query("WITH EI AS (SELECT N.ID AS ENTITY_ID, N.PARENT_ID, N.NODE_TYPE, N.CREATED_BY FROM JDONODE N WHERE N.ID IN(:entityIds)), EAR AS (WITH RECURSIVE EAR (ENTITY_ID, PARENT_ID, REQUIREMENT_ID, DISTANCE) AS ( SELECT EI.ENTITY_ID, EI.PARENT_ID, NAR.REQUIREMENT_ID, 1 FROM EI LEFT JOIN NODE_ACCESS_REQUIREMENT NAR ON (EI.ENTITY_ID = NAR.SUBJECT_ID AND NAR.SUBJECT_TYPE = 'ENTITY')	UNION ALL SELECT EAR.ENTITY_ID, N.PARENT_ID, NAR.REQUIREMENT_ID, EAR.DISTANCE+ 1 FROM JDONODE AS N JOIN EAR ON (N.ID = EAR.PARENT_ID) LEFT JOIN NODE_ACCESS_REQUIREMENT NAR ON (N.ID = NAR.SUBJECT_ID AND NAR.SUBJECT_TYPE = 'ENTITY') WHERE N.ID IS NOT NULL AND DISTANCE < 100) SELECT distinct ENTITY_ID, REQUIREMENT_ID FROM EAR WHERE REQUIREMENT_ID IS NOT NULL ORDER BY ENTITY_ID, DISTANCE), APS AS ( SELECT EAR.*, if(AA.STATE = 'APPROVED', TRUE, FALSE) AS APPROVED FROM EAR LEFT JOIN ACCESS_APPROVAL AA ON (EAR.REQUIREMENT_ID = AA.REQUIREMENT_ID AND AA.ACCESSOR_ID = :userId AND AA.STATE = 'APPROVED')) SELECT EI.ENTITY_ID, EI.NODE_TYPE, EI.CREATED_BY, APS.REQUIREMENT_ID, APS.APPROVED, AR.CONCRETE_TYPE AS REQUIREMENT_TYPE FROM EI LEFT JOIN APS ON (EI.ENTITY_ID = APS.ENTITY_ID) LEFT JOIN ACCESS_REQUIREMENT AR ON (APS.REQUIREMENT_ID = AR.ID)", params, new RowCallbackHandler() {
			...
			public void processRow(ResultSet rs) throws SQLException {
				...
				status.addRestrictionStatus(new RestrictionStatus(requirementId, requirementType, !approved));
			}
		});
