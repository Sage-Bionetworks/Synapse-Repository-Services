...
public class UserGroupCacheImpl implements UserGroupCache {
	...
	public Long getIdForUserGroupName(String name) throws NotFoundException {
		...
		if(id == null){
			try{
				id = simpleJdbcTemplate.queryForLong(SQL_GET_ID_FOR_NAME, name);
			}catch(Exception e){
	...
	public String getUserGroupNameForId(Long id) throws NotFoundException {
		...
		if(name == null){
			try{
				name = simpleJdbcTemplate.queryForObject("SELECT "+SqlConstants.COL_USER_GROUP_NAME+" FROM "+SqlConstants.TABLE_USER_GROUP+" WHERE "+SqlConstants.COL_USER_GROUP_ID+" = ?", String.class, id);
			}catch(Exception e){
				throw new NotFoundException("Could not find a principal named: "+name);
			}
			mapIdToName.put(id, name);
