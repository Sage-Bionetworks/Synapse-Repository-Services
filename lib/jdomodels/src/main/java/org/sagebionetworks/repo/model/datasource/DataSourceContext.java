 ...
 * Can be used to annotate a bean so that all the public methods will set the provided {@link DataSourceType}. 
 * Note that this will bound the type to the current thread and after each method execution the type will be cleared, 
 * it must be used with care to set this only on the calling manager or service without the need to propagate it to the dao layer.
 */
@Documented
@Inherited
@Retention(RUNTIME)
