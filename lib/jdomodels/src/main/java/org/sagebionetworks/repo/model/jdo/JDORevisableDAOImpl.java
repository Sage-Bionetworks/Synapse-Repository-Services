...
abstract public class JDORevisableDAOImpl<S extends Revisable, T extends JDORevisable<T>>
		extends JDOBaseDAOImpl<S, T> implements RevisableDAO<S> {
	
	public JDORevisableDAOImpl(String userId) {super(userId);}
...
//			pm.makePersistent(newRevision); // TODO try removing this line, which should be unnecessary
	...
	public T getLatest(PersistenceManager pm, T someRev) throws DatastoreException {
		...
		query.declareVariables(JDORevision.class.getName() + " r");
...
//				preDelete(pm, jdo);
	...
	public List<S> getInRange(int start, int end) throws DatastoreException {
		...
		try {
			...
			query.declareVariables(JDORevision.class.getName()
					+ " vRevision");
