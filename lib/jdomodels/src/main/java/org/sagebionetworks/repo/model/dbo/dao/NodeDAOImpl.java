...
public class NodeDAOImpl implements NodeDAO, InitializingBean {
	...
	/**
	 * MySQL have a default limit on the maximum recursion calls that can be made on
	 * a recursive CTE
	 */
	...
	private static final String SQL_UPDATE_ANNOTATIONS_FORMAT = "UPDATE " + TABLE_REVISION + " SET %s" + " = ? WHERE "
			+ COL_REVISION_OWNER_NODE + " = ? AND " + COL_REVISION_NUMBER + " = ?";

	private static final String SQL_UPDATE_USER_ANNOTATIONS = "UPDATE " + TABLE_REVISION + " SET "
			+ COL_REVISION_USER_ANNOS_JSON + " = ? WHERE " + COL_REVISION_OWNER_NODE + " = ? AND " + COL_REVISION_NUMBER
			+ " = ?";
	...
	private static final String SQL_TOUCH_REVISION = "UPDATE " + TABLE_REVISION + " SET " + COL_REVISION_MODIFIED_BY
			...
			+ COL_REVISION_NUMBER + " = ?";
	private static final String SQL_TOUCH_ETAG = "UPDATE " + TABLE_NODE + " SET " + COL_NODE_ETAG + " = ? WHERE "
			+ COL_NODE_ID + " = ?";
	...
	private static final String SQL_SELECT_CHILD_CRC32 = "SELECT " + COL_NODE_PARENT_ID + "," + " SUM(CRC32(CONCAT("
			+ COL_NODE_ID + ",'-'," + COL_NODE_ETAG + ",'-'," + FUNCTION_GET_ENTITY_BENEFACTOR_ID + "(" + COL_NODE_ID
			+ ")" + "))) AS 'CRC'" + " FROM " + TABLE_NODE + " WHERE " + COL_NODE_PARENT_ID + " IN(:" + BIND_PARENT_ID
			+ ")" + " GROUP BY " + COL_NODE_PARENT_ID;

	private static final String SQL_SELECT_CHILDREN_ID_AND_ETAG = "SELECT " + COL_NODE_ID + ", " + COL_NODE_ETAG + ", "
			+ FUNCTION_GET_ENTITY_BENEFACTOR_ID + "(" + COL_NODE_ID + ")" + " FROM " + TABLE_NODE + " WHERE "
			+ COL_NODE_PARENT_ID + " = ?";

	private static final String SQL_SELECT_CHILD = "SELECT " + COL_NODE_ID + " FROM " + TABLE_NODE + " WHERE "
			+ COL_NODE_PARENT_ID + " = :" + COL_NODE_PARENT_ID + " AND " + COL_NODE_NAME + " = :" + COL_NODE_NAME;

	private static final String SQL_SELECT_CHILDREN = "SELECT" + " " + COL_NODE_ID + ", " + COL_NODE_TYPE + " FROM "
			+ TABLE_NODE + " WHERE " + COL_NODE_PARENT_ID + " = ?" + " LIMIT ? OFFSET ?";

	private static final String SQL_COUNT_CHILDREN = "SELECT COUNT(" + COL_NODE_ID + ")" + " FROM " + TABLE_NODE + ""
			+ " WHERE " + COL_NODE_PARENT_ID + " = ?";

	private static final String SELECT_PROJECTS_STATS = "SELECT n." + COL_NODE_ID + ", n." + COL_NODE_NAME + ", n."
			+ COL_NODE_TYPE + ", COALESCE(ps." + COL_PROJECT_STAT_LAST_ACCESSED + ", n." + COL_NODE_CREATED_ON + ") AS "
			+ COL_PROJECT_STAT_LAST_ACCESSED + ", r." + COL_REVISION_MODIFIED_BY + ", r." + COL_REVISION_MODIFIED_ON
			+ " FROM " + TABLE_NODE + " n JOIN " + TABLE_REVISION + " r ON n." + COL_NODE_ID + " = r."
			+ COL_REVISION_OWNER_NODE + " AND n." + COL_NODE_CURRENT_REV + " = r." + COL_REVISION_NUMBER + " LEFT JOIN "
			+ TABLE_PROJECT_STAT + " ps ON n." + COL_NODE_ID + " = ps." + COL_PROJECT_STAT_PROJECT_ID + " AND ps."
			+ COL_PROJECT_STAT_USER_ID + " = :" + BIND_PROJECT_STAT_USER_ID + " WHERE n." + COL_NODE_TYPE + " = '"
			+ EntityType.project.name() + "' AND n." + COL_NODE_ID + " IN (:" + BIND_NODE_IDS + ")";

	private static final String BENEFACTOR_ALIAS = "BENEFACTOR";
	private static final String SQL_SELECT_BENEFACTOR_N = FUNCTION_GET_ENTITY_BENEFACTOR_ID + "(N." + COL_NODE_ID
			+ ") AS " + BENEFACTOR_ALIAS;

	private static final String SQL_SELECT_BENEFACTORS = "SELECT N." + COL_NODE_ID + ", " + SQL_SELECT_BENEFACTOR_N
			+ " FROM " + TABLE_NODE + " N" + " WHERE N." + COL_NODE_ID + " IN (:" + BIND_NODE_IDS + ")";
	...
	private static final String JOIN_NODE_REVISION = TABLE_NODE + " N" + " JOIN " + TABLE_REVISION + " R" + " ON (N."
			+ COL_NODE_ID + " = R." + COL_REVISION_OWNER_NODE + " AND N." + COL_NODE_CURRENT_REV + " = R."
			+ COL_REVISION_NUMBER + ")";
	private static final String JOIN_NODE_REVISION_FILES = JOIN_NODE_REVISION + " LEFT JOIN " + TABLE_FILES
			+ " F ON (R." + COL_REVISION_FILE_HANDLE_ID + " = F." + COL_FILES_ID + " )";

	private static final String SQL_SELECT_CHIDREN_TEMPLATE = ENTITY_HEADER_SELECT + " FROM " + JOIN_NODE_REVISION
			+ " WHERE N." + COL_NODE_PARENT_ID + " = :" + BIND_PARENT_ID + " %1$s" + " AND N." + COL_NODE_TYPE
			+ " IN (:" + BIND_NODE_TYPES + ")" + " ORDER BY %2$s %3$s" + " LIMIT :" + BIND_LIMIT + " OFFSET :"
			+ BIND_OFFSET;

	private static final String SQL_SELECT_CHIDREN_STATS = "SELECT COUNT(*), SUM(F." + COL_FILES_CONTENT_SIZE + ")"
			+ " FROM " + TABLE_NODE + " N" + " JOIN " + TABLE_REVISION + " R" + " ON (N." + COL_NODE_ID + " = R."
			+ COL_REVISION_OWNER_NODE + " AND N." + COL_NODE_CURRENT_REV + " = R." + COL_REVISION_NUMBER + ")"
			+ " LEFT JOIN " + TABLE_FILES + " F" + " ON (R." + COL_REVISION_FILE_HANDLE_ID + " = F." + COL_FILES_ID
			+ ")" + " WHERE N." + COL_NODE_PARENT_ID + " = :" + BIND_PARENT_ID + " %1$s" + " AND N." + COL_NODE_TYPE
			+ " IN (:" + BIND_NODE_TYPES + ")";
	...
	private static final String SQL_SELECT_NODE_ID_BY_ALIAS = "SELECT " + COL_NODE_ID + " FROM " + TABLE_NODE
			+ " WHERE " + COL_NODE_ALIAS + " = ?";

	private static final String SQL_SELECT_ALIAS_BY_NODE_ID = "SELECT " + COL_NODE_ID + ", " + COL_NODE_ALIAS + " FROM "
			+ TABLE_NODE + " WHERE " + COL_NODE_ID + " IN (:" + BIND_NODE_IDS + ")";

	private static final String SELECT_ENTITY_HEADERS_FOR_ENTITY_IDS = ENTITY_HEADER_SELECT + " FROM " + TABLE_NODE
			+ " N" + " JOIN " + TABLE_REVISION + " R" + " ON (N." + COL_NODE_ID + " = R." + COL_REVISION_OWNER_NODE
			+ " AND N." + COL_NODE_CURRENT_REV + " = R." + COL_REVISION_NUMBER + ")" + " WHERE " + COL_NODE_ID
			+ " IN (:nodeIds)";

	private static final String PARAM_NAME_IDS = "ids_param";

	private static final String SQL_SELECT_REV_FILE_HANDLE_ID = "SELECT " + COL_REVISION_FILE_HANDLE_ID + " FROM "
			+ TABLE_REVISION + " WHERE " + COL_REVISION_OWNER_NODE + " = ? AND " + COL_REVISION_NUMBER + " = ?";
	private static final String SELECT_ANNOTATIONS_ONLY_SELECT_CLAUSE_PREFIX = "SELECT N." + COL_NODE_ID + ", N."
			+ COL_NODE_ETAG + ", N." + COL_NODE_CREATED_ON + ", N." + COL_NODE_CREATED_BY + ", R.";

	private static final String SELECT_ANNOTATIONS_ONLY_FROM_AND_WHERE_CLAUSE_PREFIX = " FROM  " + TABLE_NODE + " N, "
			+ TABLE_REVISION + " R WHERE N." + COL_NODE_ID + " = :" + COL_NODE_ID + " AND R." + COL_REVISION_OWNER_NODE
			+ " = N." + COL_NODE_ID + " AND R." + COL_REVISION_NUMBER + "=";
	private static final String SELECT_USER_ANNOTATIONS_ONLY_PREFIX = "SELECT N." + COL_NODE_ID + ", N." + COL_NODE_ETAG
			+ ", R." + COL_REVISION_USER_ANNOS_JSON + " FROM  " + TABLE_NODE + " N, " + TABLE_REVISION + " R WHERE N."
			+ COL_NODE_ID + " = ? AND R." + COL_REVISION_OWNER_NODE + " = N." + COL_NODE_ID + " AND R."
			+ COL_REVISION_NUMBER + " = ";
	...
	private static final String GET_CURRENT_REV_NUMBER_SQL = "SELECT " + COL_NODE_CURRENT_REV + " FROM " + TABLE_NODE
			+ " WHERE " + COL_NODE_ID + " = ?";
	private static final String GET_NODE_TYPE_SQL = "SELECT " + COL_NODE_TYPE + " FROM " + TABLE_NODE + " WHERE "
			+ COL_NODE_ID + " = ?";
	private static final String GET_REV_ACTIVITY_ID_SQL = "SELECT " + COL_REVISION_ACTIVITY_ID + " FROM "
			+ TABLE_REVISION + " WHERE " + COL_REVISION_OWNER_NODE + " = ? AND " + COL_REVISION_NUMBER + " = ?";
	private static final String GET_NODE_CREATED_BY_SQL = "SELECT " + COL_NODE_CREATED_BY + " FROM " + TABLE_NODE
			+ " WHERE " + COL_NODE_ID + " = ?";
	private static final String SQL_SELECT_PARENT_TYPE_NAME = "SELECT " + COL_NODE_ID + ", " + COL_NODE_PARENT_ID + ", "
			+ COL_NODE_TYPE + ", " + COL_NODE_NAME + " FROM " + TABLE_NODE + " WHERE " + COL_NODE_ID + " = ?";
	private static final String SQL_GET_ALL_CHILDREN_IDS = "SELECT " + COL_NODE_ID + " FROM " + TABLE_NODE + " WHERE "
			+ COL_NODE_PARENT_ID + " = ? ORDER BY " + COL_NODE_ID;
	...
	private static final String SQL_SELECT_ENTITY_DTO = "SELECT N." + COL_NODE_ID + ", N." + COL_NODE_CURRENT_REV
			+ ", N." + COL_NODE_CREATED_BY + ", N." + COL_NODE_CREATED_ON + ", N." + COL_NODE_ETAG + ", N."
			+ COL_NODE_NAME + ", N." + COL_NODE_TYPE + ", N." + COL_NODE_PARENT_ID + ", " + BENEFACTOR_FUNCTION_ALIAS
			+ ", " + PROJECT_FUNCTION_ALIAS + ", R." + COL_REVISION_MODIFIED_BY + ", R." + COL_REVISION_MODIFIED_ON
			+ ", R." + COL_REVISION_FILE_HANDLE_ID + ", R." + COL_REVISION_USER_ANNOS_JSON + ", F."
			+ COL_FILES_CONTENT_SIZE + ", F." + COL_FILES_BUCKET_NAME + ", F." + COL_FILES_CONTENT_MD5 + " FROM "
			+ JOIN_NODE_REVISION_FILES + " WHERE N." + COL_NODE_ID + " IN(:" + NODE_IDS_LIST_PARAM_NAME
			+ ") ORDER BY N." + COL_NODE_ID + " ASC";

	private static final String SQL_GET_CURRENT_VERSIONS = "SELECT " + COL_NODE_ID + "," + COL_NODE_CURRENT_REV
			+ " FROM " + TABLE_NODE + " WHERE " + COL_NODE_ID + " IN ( :" + NODE_IDS_LIST_PARAM_NAME + " )";
	private static final String OWNER_ID_PARAM_NAME = "OWNER_ID";

	private static final String LAST_ACCESSED_OR_CREATED = "coalesce(ps." + COL_PROJECT_STAT_LAST_ACCESSED + ", n."
			+ COL_NODE_CREATED_ON + ")";

	private static final String BIND_CREATED_BY = "bCreatedBy";
	private static final String SELECT_CREATED = " AND n." + COL_NODE_CREATED_BY + " = :" + BIND_CREATED_BY;
	private static final String SELECT_NOT_CREATED = " AND n." + COL_NODE_CREATED_BY + " <> :" + BIND_CREATED_BY;

	private static final String SELECT_PROJECTS_ORDER = " ORDER BY " + LAST_ACCESSED_OR_CREATED;

	private static final String SELECT_NAME_ORDER = " ORDER BY n." + COL_NODE_NAME;

	/**
	 * To determine if a node has children we fetch the first child ID.
	 */
	private static final String SQL_GET_FIRST_CHILD = "SELECT " + COL_NODE_ID + " FROM " + TABLE_NODE + " WHERE "
			+ COL_NODE_PARENT_ID + " = ? LIMIT 1 OFFSET 0";

	private static final String SQL_GET_ALL_VERSION_INFO_PAGINATED = "SELECT rr." + COL_REVISION_NUMBER + ", rr."
			+ COL_REVISION_LABEL + ", rr." + COL_REVISION_COMMENT + ", rr." + COL_REVISION_MODIFIED_BY + ", rr."
			+ COL_REVISION_MODIFIED_ON + ", ff." + COL_FILES_CONTENT_MD5 + ", ff." + COL_FILES_CONTENT_SIZE + " FROM "
			+ TABLE_REVISION + " rr left outer join " + TABLE_FILES + " ff on (rr." + COL_REVISION_FILE_HANDLE_ID
			+ " = ff." + COL_FILES_ID + ") WHERE rr." + COL_REVISION_OWNER_NODE + " = :" + OWNER_ID_PARAM_NAME
			+ " ORDER BY rr." + COL_REVISION_NUMBER + " DESC LIMIT :" + LIMIT_PARAM_NAME + " OFFSET :"
			+ OFFSET_PARAM_NAME;

	/**
	 * The max number of entity versions a MD5 string can map to. This puts a check
	 * to potential DDOS attacks via MD5. We retrieve at most MD5_LIMIT + 1 rows. If
	 * the number of rows retrieved is > MD5_LIMIT, an exception is thrown.
	 */
	private static final int NODE_VERSION_LIMIT_BY_FILE_MD5 = 200;
	private static final String SELECT_NODE_VERSION_BY_FILE_MD5 = ENTITY_HEADER_SELECT + " FROM " + TABLE_REVISION
			+ " R, " + TABLE_FILES + " F, " + TABLE_NODE + " N" + " WHERE R." + COL_REVISION_OWNER_NODE + " = N."
			+ COL_NODE_ID + " AND  R." + COL_REVISION_FILE_HANDLE_ID + " = F." + COL_FILES_ID + " AND F."
			+ COL_FILES_CONTENT_MD5 + " = :" + COL_FILES_CONTENT_MD5 + " LIMIT " + (NODE_VERSION_LIMIT_BY_FILE_MD5 + 1);
	...
	public static final String PATH_QUERY_TEMPLATE = "WITH RECURSIVE PATH (" + COL_NODE_ID + ", " + COL_NODE_NAME + ", "
			...
			+ " AS N WHERE " + COL_NODE_ID + " = ?" + " UNION ALL" + " SELECT N." + COL_NODE_ID + ", N." + COL_NODE_NAME
			+ ", N." + COL_NODE_TYPE + ", N." + COL_NODE_PARENT_ID + ", PATH.DISTANCE+ 1 FROM " + TABLE_NODE
			+ " AS N JOIN PATH ON (N." + COL_NODE_ID + " = PATH." + COL_NODE_PARENT_ID + ")" + " WHERE N." + COL_NODE_ID
			+ " IS NOT NULL AND DISTANCE < " + MAX_PATH_DEPTH + " )" + " SELECT %1s FROM PATH ORDER BY DISTANCE DESC";

	private static final String SQL_STRING_CONTAINERS_TYPES = String.join(",", "'" + EntityType.project.name() + "'",
	...
	private static final String SQL_ETAG_WITHOUT_LOCK = "SELECT " + COL_NODE_ETAG + " FROM " + TABLE_NODE
			+ " WHERE ID = ?";
	private static final String SQL_ETAG_FOR_UPDATE = SQL_ETAG_WITHOUT_LOCK + " FOR UPDATE";

	private static final String SQL_GET_ALL_VERSION_NUMBERS = "SELECT " + COL_REVISION_NUMBER + " FROM "
			+ TABLE_REVISION + " WHERE " + COL_REVISION_OWNER_NODE + " = ? ORDER BY " + COL_REVISION_NUMBER + " DESC";

	private static final String SQL_GET_LATEST_VERSION_NUMBER = "SELECT MAX(" + COL_REVISION_NUMBER + ") FROM "
			+ TABLE_REVISION + " WHERE " + COL_REVISION_OWNER_NODE + " = ?";
	...
	private static final String SQL_COUNT_NODE_ID = "SELECT COUNT(" + COL_NODE_ID + ") FROM " + TABLE_NODE + " WHERE "
			+ COL_NODE_ID + " = :" + BIND_ID_KEY;
	private static final String SQL_COUNT_REVISON_ID = "SELECT COUNT(" + COL_REVISION_OWNER_NODE + ") FROM "
			+ TABLE_REVISION + " WHERE " + COL_REVISION_OWNER_NODE + " = ? AND " + COL_REVISION_NUMBER + " = ?";

	private static final String SQL_GET_FILE_HANDLE_IDS = "SELECT DISTINCT " + COL_REVISION_FILE_HANDLE_ID + " FROM "
			+ TABLE_REVISION + " WHERE " + COL_REVISION_OWNER_NODE + " = ?";
	...
	/**
	 * Create a new node with an ID. s
	 * 
	 * @param dto
	 * @return
	...
	private Node create(Node dto) {
		...
		transactionalMessenger.sendMessageAfterCommit(new MessageToSend().withObservableEntity(dboNode)
				.withChangeType(ChangeType.CREATE).withUserId(dboNode.getCreatedBy()));
		...
		try {
			...
			checkExceptionDetails(dboNode.getName(), dboNode.getAlias(), KeyFactory.keyToString(dboNode.getParentId()),
					e);
		}
	...
	/**
	 * Determine which constraint was violated and throw a more meaningful
	 * exception.
	 * 
	 * @param dto
	 * @param node
	...
	private void checkExceptionDetails(String name, String alias, String parentId, IllegalArgumentException e) {
		if (e.getMessage().indexOf(CONSTRAINT_UNIQUE_CHILD_NAME) > 0)
			throw new NameConflictException(
					"An entity with the name: " + name + " already exists with a parentId: " + parentId);
		if (e.getMessage().indexOf(CONSTRAINT_UNIQUE_ALIAS) > 0)
			throw new NameConflictException(
					"The friendly url name (alias): " + alias + " is already taken.  Please select another.");
		throw e;
	}
	...
	public Long createNewVersion(Node newVersion) {
		...
		// Avoid recycling the revision numbers (See PLFM-3781) and uses the current max
		// revision
		...
		if (newVersion.getVersionLabel() == null) {
			// This is a fix for PLFM-995. This was modified not to use the KeyFactory
			// because
	...
	public Node getNodeForVersion(String id, Long versionNumber) {
		...
		if (versionNumber == null)
			throw new IllegalArgumentException("Version number cannot be null");
		try {
			return this.jdbcTemplate.queryForObject(SQL_SELECT_NODE_VERSION, NODE_MAPPER, versionNumber,
	...
	public List<Long> getSubTreeNodeIdsOrderByDistanceDesc(Long parentId, int limit) {
		return jdbcTemplate.queryForList("WITH RECURSIVE NODES (ID, DISTANCE) AS (" + " SELECT " + COL_NODE_ID
				+ ", 1 FROM " + TABLE_NODE + " WHERE " + COL_NODE_PARENT_ID + " = ?" + " UNION" + " SELECT N."
				+ COL_NODE_ID + ", C.DISTANCE + 1" + " FROM NODES AS C JOIN " + TABLE_NODE + " AS N ON C." + COL_NODE_ID
				+ " = N." + COL_NODE_PARENT_ID + " AND C.DISTANCE < " + MAX_PATH_RECURSION + ")"
	...
	public void deleteVersion(String nodeId, Long versionNumber) {
		...
		// Delete the revision.
		boolean wasDeleted = dboBasicDao.deleteObjectByPrimaryKey(DBORevision.class,
				getRevisionParameters(id, versionNumber));
		if (wasDeleted) {
			// Make sure a revision still exists
			Long latestVersion = getLatestVersionNumber(nodeId)
					.orElseThrow(() -> new IllegalArgumentException("Cannot delete the last version of a node"));
			...
			// Note: we do not change the maxRevNumber so that the old versions are not
			// recycled (See PLFM-3781)
	...
	public EntityType getNodeTypeById(String nodeId) {
		...
		try {
			String typeString = this.jdbcTemplate.queryForObject(GET_NODE_TYPE_SQL, String.class,
	...
	@Override
	public org.sagebionetworks.repo.model.Annotations getEntityPropertyAnnotationsForVersion(final String id,
			Long versionNumber) {
		ValidateArgument.requiredNotEmpty(id, "id");
	...
	private org.sagebionetworks.repo.model.Annotations getAnnotations(final String id, Long version,
			final String annotationsColumnName) {
		...
		try {
			org.sagebionetworks.repo.model.Annotations annos = namedParameterJdbcTemplate.queryForObject(sql.toString(),
					parameters, new AnnotationsRowMapper(annotationsColumnName));
	...
	/**
	 * A RowMapper that extracts Annotation from a result set. The result set must
	 * include COL_REVISION_USER_ANNOS_JSON, COL_NODE_CREATED_ON, COL_NODE_ID,
	 * COL_NODE_CREATED_BY
	 *
	 */
	...
	/**
	 * A RowMapper that extracts Annotation from a result set. The result set must
	 * COL_REVISION_ANNOS_BLOB, COL_NODE_ETAG, COL_NODE_CREATED_ON, COL_NODE_ID,
	 * COL_NODE_CREATED_BY
	 *
	 */
	...
	public Annotations getUserAnnotations(String id) {
		...
		// Select just the references, not the entire node.
		try {
			return jdbcTemplate.queryForObject(SELECT_USER_ANNOTATIONS_ONLY_PREFIX + "N." + COL_NODE_CURRENT_REV,
					ANNOTATIONS_V2_ROW_MAPPER, KeyFactory.stringToKey(id));
		} catch (EmptyResultDataAccessException e) {
			// Occurs if there are no results
			throw new NotFoundException(CANNOT_FIND_A_NODE_WITH_ID + id);
		}
	...
	public Annotations getUserAnnotationsForVersion(final String id, Long versionNumber) {
		...
		// Select just the references, not the entire node.
		try {
			Annotations userAnnotations = jdbcTemplate.queryForObject(SELECT_USER_ANNOTATIONS_ONLY_PREFIX + "?",
	...
	@WriteTransaction
	@Override
	public void updateEntityPropertyAnnotations(String nodeId, org.sagebionetworks.repo.model.Annotations updatedAnnos)
			throws NotFoundException, DatastoreException {
		updateAnnotations(nodeId, updatedAnnos, COL_REVISION_ENTITY_PROPERTY_ANNOTATIONS_BLOB);
	}

	void updateAnnotations(String nodeId, org.sagebionetworks.repo.model.Annotations updatedAnnos,
			String annotationColumnName) throws NotFoundException, DatastoreException {
		...
		try {
			...
			this.jdbcTemplate.update(String.format(SQL_UPDATE_ANNOTATIONS_FORMAT, annotationColumnName), newAnnos,
					nodeIdLong, currentRevision);
		} catch (IOException e) {
			throw new DatastoreException(e);
		}
	}
	...
	public Optional<Long> getLatestVersionNumber(String id) {
		Long latestVersion = jdbcTemplate.queryForObject(SQL_GET_LATEST_VERSION_NUMBER, Long.class,
				KeyFactory.stringToKey(id));
		return Optional.ofNullable(latestVersion);
	}

	@Override
	public List<VersionInfo> getVersionsOfEntity(final String entityId, long offset, long limit) {
		MapSqlParameterSource params = new MapSqlParameterSource();
		...
		return namedParameterJdbcTemplate.query(SQL_GET_ALL_VERSION_INFO_PAGINATED, params,
	...
	public boolean doesNodeRevisionExist(String nodeId, Long revNumber) {
		try {
			long count = jdbcTemplate.queryForObject(SQL_COUNT_REVISON_ID, Long.class, KeyFactory.stringToKey(nodeId),
					revNumber);
			return count > 0;
		} catch (Exception e) {
			// Can occur when the schema does not exist.
	...
	public List<EntityHeader> getEntityHeader(Set<Long> entityIds) {
		...
		return namedParameterJdbcTemplate.query(SELECT_ENTITY_HEADERS_FOR_ENTITY_IDS, namedParameters,
				ENTITY_HEADER_ROWMAPPER);
	}
	...
	public List<EntityHeader> getEntityHeaderByMd5(String md5) throws DatastoreException, NotFoundException {
		...
		List<EntityHeader> rowList = namedParameterJdbcTemplate.query(SELECT_NODE_VERSION_BY_FILE_MD5, paramMap,
				ENTITY_HEADER_ROWMAPPER);

		if (rowList.size() > NODE_VERSION_LIMIT_BY_FILE_MD5) {
			throw new DatastoreException(
					"MD5 " + md5 + " maps to more than " + NODE_VERSION_LIMIT_BY_FILE_MD5 + " entity versions.");
		}
		return rowList;
	}
	...
	public String getNodeIdForPath(String path) throws DatastoreException {
		...
		try {
			...
			if (list == null || list.size() < 1)
				return null;
			if (list.size() > 1)
				throw new IllegalStateException("Found more than one node with a path: " + path);
	...
	public List<String> getChildrenIdsAsList(String id) throws DatastoreException {
		...
		List<Map<String, Object>> restuls = jdbcTemplate.queryForList(SQL_GET_ALL_CHILDREN_IDS,
				KeyFactory.stringToKey(id));
	...
	public Long getCurrentRevisionNumber(String nodeId) {
		if (nodeId == null)
			throw new IllegalArgumentException("Node Id cannot be null");
		try {
			return this.jdbcTemplate.queryForObject(GET_CURRENT_REV_NUMBER_SQL, Long.class,
					KeyFactory.stringToKey(nodeId));
		} catch (EmptyResultDataAccessException e) {
			throw new NotFoundException(ERROR_RESOURCE_NOT_FOUND);
		}
	...
	public String getActivityId(String nodeId, Long revNumber) {
		if (nodeId == null)
			throw new IllegalArgumentException("Node Id cannot be null");
		try {
			Long activityId = this.jdbcTemplate.queryForObject(GET_REV_ACTIVITY_ID_SQL, Long.class,
					KeyFactory.stringToKey(nodeId), revNumber);
	...
	public Long getCreatedBy(String nodeId) {
		if (nodeId == null)
			throw new IllegalArgumentException("Node Id cannot be null");
		try {
			return this.jdbcTemplate.queryForObject(GET_NODE_CREATED_BY_SQL, Long.class,
					KeyFactory.stringToKey(nodeId));
		} catch (EmptyResultDataAccessException e) {
			throw new NotFoundException(ERROR_RESOURCE_NOT_FOUND);
		}
	...
	public String getFileHandleIdForVersion(String id, Long versionNumber) {
		...
		try {
			Long handleId = jdbcTemplate.queryForObject(SQL_SELECT_REV_FILE_HANDLE_ID, Long.class, nodeId,
					versionNumber);
			if (handleId != null) {
	...
	public List<FileHandleAssociation> getFileHandleAssociationsForCurrentVersion(List<String> entityIds) {
		...
		return namedParameterJdbcTemplate.query(SELECT_CURRENT_VERSION_FILE_HANDLES, parameters,
				new RowMapper<FileHandleAssociation>() {
	...
	public List<Reference> getCurrentRevisionNumbers(List<String> nodeIds) {
		...
		List<Reference> refs = this.namedParameterJdbcTemplate.query(SQL_GET_CURRENT_VERSIONS, parameters,
	...
	@Override
	public List<ProjectHeader> getProjectHeaders(Long userId, Set<Long> projectIds, ProjectListType type,
			ProjectListSortColumn sortColumn, SortDirection sortDirection, Long limit, Long offset) {
	 ...
	 * @return
	 */
	public static String getProjectStatAdditionalCondition(Map<String, Object> parameters, Long userId,
	 ...
	 * @return
	 */
	public static String getProjectStatsOderByAndPaging(Map<String, Object> parameters,
			ProjectListSortColumn sortColumn, SortDirection sortDirection, Long limit, Long offset) {
	...
	/**
	 * As part of PLFM-6061, the implementation of this method was changed from n
	 * number of SQL calls to a single 'WITH RECURSIVE' call. The change eliminates
	 * sending intermediate results back and forth from the server and database.
	 */
	@Override
	public Set<Long> getAllContainerIds(Collection<Long> parentIds, int maxNumberIds) throws LimitExceededException {
		...
		List<Long> children = namedParameterJdbcTemplate.queryForList("WITH RECURSIVE CONTAINERS (ID) AS (" + " SELECT "
				+ COL_NODE_ID + " FROM " + TABLE_NODE + " WHERE " + COL_NODE_ID + " IN (:" + PARAM_NAME_IDS + ")"
				+ " AND " + COL_NODE_TYPE + " IN (" + SQL_STRING_CONTAINERS_TYPES + ")" + " UNION DISTINCT"
				+ " SELECT N." + COL_NODE_ID + " FROM CONTAINERS AS C JOIN " + TABLE_NODE + " AS N ON (C." + COL_NODE_ID
				+ " = N." + COL_NODE_PARENT_ID + " AND N." + COL_NODE_TYPE + " IN (" + SQL_STRING_CONTAINERS_TYPES
				+ "))" + ")" + "SELECT ID FROM CONTAINERS LIMIT :" + BIND_LIMIT, parameters, Long.class);
		Set<Long> finalSet = new HashSet<>(children);
		if (finalSet.size() > maxNumberIds || children.size() + results.size() > maxNumberIds) {
	 ...
	 * 
	 * @see
	 * org.sagebionetworks.repo.model.NodeDAO#getAllContainerIds(java.lang.String)
	 */
	@Override
	...
	public List<IdAndAlias> getAliasByNodeId(List<String> nodeIds) {
		...
		return namedParameterJdbcTemplate.query(SQL_SELECT_ALIAS_BY_NODE_ID, params, new RowMapper<IdAndAlias>() {
			@Override
			public IdAndAlias mapRow(ResultSet rs, int rowNum) throws SQLException {
	...
	public String getProjectId(String nodeId) {
		...
		if (projectId == null) {
			/*
			 * ProjectId will be null if the node does not exist or if the node is in the
			 * trash. In either case a NotFoundException should be thrown.
			 */
			throw new NotFoundException(ERROR_RESOURCE_NOT_FOUND);
		} else if (projectId < 0) {
	...
	public List<ObjectDataDTO> getEntityDTOs(List<Long> ids, final int maxAnnotationSize) {
		...
		return namedParameterJdbcTemplate.query(SQL_SELECT_ENTITY_DTO, parameters, new RowMapper<ObjectDataDTO>() {
			...
			@Override
			public ObjectDataDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
	...
	public List<EntityHeader> getChildren(String parentId, List<EntityType> includeTypes, Set<Long> childIdsToExclude,
		...
		String sql = String.format(SQL_SELECT_CHIDREN_TEMPLATE, getFragmentExcludeNodeIds(childIdsToExclude),
				getFragmentSortColumn(sortBy), sortDirection.name());
	...
	public List<NodeIdAndType> getChildren(String parentId, long limit, long offset) {
		...
		return jdbcTemplate.query(SQL_SELECT_CHILDREN, new RowMapper<NodeIdAndType>() {
			...
			@Override
			public NodeIdAndType mapRow(ResultSet rs, int rowNum) throws SQLException {
	...
	public List<IdAndEtag> getChildren(long parentId) {
		...
		return jdbcTemplate.query(SQL_SELECT_CHILDREN_ID_AND_ETAG, new RowMapper<IdAndEtag>() {
			@Override
			public IdAndEtag mapRow(ResultSet rs, int rowNum) throws SQLException {
	...
	public String getNodeName(String nodeId) {
		...
		try {
			return this.jdbcTemplate.queryForObject(
					"SELECT " + COL_NODE_NAME + " FROM " + TABLE_NODE + " WHERE " + COL_NODE_ID + " =? ", String.class,
					KeyFactory.stringToKey(nodeId));
		} catch (EmptyResultDataAccessException e) {
			throw new NotFoundException();
		}
