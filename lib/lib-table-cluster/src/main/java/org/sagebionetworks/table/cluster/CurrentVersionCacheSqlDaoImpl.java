...
public class CurrentVersionCacheSqlDaoImpl implements CurrentVersionCacheDao {
	private static Logger log = LogManager.getLogger(CurrentVersionCacheSqlDaoImpl.class);
	 ...
	 */
	public CurrentVersionCacheSqlDaoImpl(DataSource dataSource) {
	...
	private static TransactionTemplate createTransactionTemplate(DataSourceTransactionManager transactionManager, boolean readOnly) {
		...
		transactionDef.setName("CurrentVersionCacheSqlDaoImpl");
	...
	@Override
	public void deleteCurrentVersionTable(final Long tableId) {
		tryAndIgnoreInWriteTransaction(new TransactionCallback<Void>() {
			@Override
			public Void doInTransaction(TransactionStatus status) {
