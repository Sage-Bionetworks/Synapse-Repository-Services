...
public class AwsKinesisFirehoseBatchClientImpl implements AwsKinesisFirehoseBatchClient {
	...
	@Autowired
	public AwsKinesisFirehoseBatchClientImpl(FirehoseClient kinesisFirehoseClient) {
	...
	private void sendBatchRequest(String streamName, List<Record> records, int retryNumber) {
		...
		PutRecordBatchResponse result = kinesisFirehoseClient.putRecordBatch(request);
		...
		if (result.failedPutCount() > 0) {
			...
			List<Record> failedRecords = new ArrayList<>(result.failedPutCount());

			int index = 0;

			for (PutRecordBatchResponseEntry recordResult : result.requestResponses()) {
				if (recordResult.errorCode() != null) {
					LOG.warn("Record delivery in a batch did not succeed, will retry: {} (Error Code: {}, Stream: {})", recordResult.errorMessage(), recordResult.errorCode(), streamName);
