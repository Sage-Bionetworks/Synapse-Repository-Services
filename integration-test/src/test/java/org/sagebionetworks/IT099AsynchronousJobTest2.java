...
/**
 * This test imitates how SWC starts and tracks its asynchronous process.
 * Since SWC starts and tracks all asynchronous process in the same way,
 * we only use startAsyncJob() and getAsyncResult() with the same request body
 * for each process.
 */
public class IT099AsynchronousJobTest2 {
	...
	public void testUploadCSVToTable() throws Exception{
		...
		try{
			...
			final String uploadPreviewToken = synapse.startAsynchJob(AsynchJobType.TableCSVUploadPreview, previewRequest);
			...
			UploadToTablePreviewResult uploadPreviewResult = TimeUtils.waitFor(MAX_WAIT_MS, 500L, new Callable<Pair<Boolean, UploadToTablePreviewResult>>() {
				...
				public Pair<Boolean, UploadToTablePreviewResult> call() throws Exception {
					try {
						UploadToTablePreviewResult uploadResult = (UploadToTablePreviewResult) synapse.getAsyncResult(
								AsynchJobType.TableCSVUploadPreview, uploadPreviewToken, previewRequest);
			...
			uploadRequest.setUpdateEtag(null);
			uploadRequest.setLinesToSkip(null);
			uploadRequest.setCsvTableDescriptor(null);
			final String uploadToken = synapse.startAsynchJob(AsynchJobType.TableCSVUpload, uploadRequest);
			try {
				synapse.getAsyncResult(AsynchJobType.TableCSVUpload, uploadToken, tableId);
			} catch (SynapseResultNotReadyException e) {
				...
				public Pair<Boolean, UploadToTableResult> call() throws Exception {
					try {
						UploadToTableResult uploadResult = 
								(UploadToTableResult) synapse.getAsyncResult(AsynchJobType.TableCSVUpload, uploadToken, tableId);
						return Pair.create(true, uploadResult);
					} catch (SynapseResultNotReadyException e) {
						return Pair.create(false, null);
					}
			...
			final DownloadFromTableRequest downloadRequest = new DownloadFromTableRequest();
			...
			downloadRequest.setCsvTableDescriptor(null);
			final String downloadToken = synapse.startAsynchJob(AsynchJobType.TableCSVDownload, downloadRequest);
			try {
				synapse.getAsyncResult(AsynchJobType.TableCSVDownload, downloadToken, downloadRequest);
			} catch (SynapseResultNotReadyException e) {
			...
			DownloadFromTableResult downloadResult = TimeUtils.waitFor(MAX_WAIT_MS, 500L,
					new Callable<Pair<Boolean, DownloadFromTableResult>>() {
						...
						public Pair<Boolean, DownloadFromTableResult> call() throws Exception {
							try {
								DownloadFromTableResult downloadResult = 
										(DownloadFromTableResult) synapse.getAsyncResult(AsynchJobType.TableCSVDownload, downloadToken, downloadRequest);
			...
			filesToDelete.add(fileHandle);
			// We now have enough to apply the data to the table
			final UploadToTableRequest uploadRequest2 = new UploadToTableRequest();
			uploadRequest.setEntityId(tableId);
			uploadRequest.setTableId(tableId);
			uploadRequest.setUploadFileHandleId(fileHandle.getId());
			...
			uploadRequest.setLinesToSkip(null);
			uploadRequest.setCsvTableDescriptor(null);
			final String updateUploadToken = synapse.startAsynchJob(AsynchJobType.TableCSVUpload, uploadRequest2);
			try {
				synapse.getAsyncResult(AsynchJobType.TableCSVUpload, updateUploadToken, uploadRequest2);
			} catch (SynapseResultNotReadyException e) {
				status = e.getJobStatus();
				assertNotNull(status);
				assertNotNull(status.getJobId());
			...
			uploadResult = TimeUtils.waitFor(MAX_WAIT_MS, 500L, new Callable<Pair<Boolean, UploadToTableResult>>() {
				...
				public Pair<Boolean, UploadToTableResult> call() throws Exception {
					try {
						UploadToTableResult uploadResult = 
								(UploadToTableResult) synapse.getAsyncResult(AsynchJobType.TableCSVUpload, updateUploadToken, uploadRequest2);
						return Pair.create(true, uploadResult);
					} catch (SynapseResultNotReadyException e) {
						return Pair.create(false, null);
					}
	...
	public RowSet waitForQuery(String sql, final String tableId) throws Exception {
		...
		query.setLimit(null);
		final QueryBundleRequest bundleRequest = new QueryBundleRequest();
		bundleRequest.setEntityId(tableId);
		bundleRequest.setQuery(query);
		bundleRequest.setPartMask((long) SynapseClient.QUERY_PARTMASK);
		final String asyncToken = synapse.startAsynchJob(AsynchJobType.TableQuery, bundleRequest);
		return TimeUtils.waitFor(MAX_WAIT_MS, 500L, new Callable<Pair<Boolean, RowSet>>() {
			...
			public Pair<Boolean, RowSet> call() throws Exception {
				try {
					QueryResultBundle result = (QueryResultBundle) synapse.getAsyncResult(AsynchJobType.TableQuery, asyncToken, bundleRequest);
