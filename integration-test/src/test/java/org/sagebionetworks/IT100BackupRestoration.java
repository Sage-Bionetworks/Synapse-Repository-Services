...
//	private static final String HTTPS = "https://";
	...
	public void createSnapshot() throws Exception{
		// Start the daemon
		JSONObject status = synapse.createEntity("/startBackupDaemon", new JSONObject());
		assertNotNull(status);
		assertNotNull(status.getString("status"));
		assertFalse(status.getString("errorMessage"), BackupRestoreStatus.STATUS.FAILED.name().equals(status.getString("status")));
		assertTrue(BackupRestoreStatus.TYPE.BACKUP.name().equals(status.getString("type")));
		...
		while(true){
			...
			assertTrue("Timed out waiting for a backup to complete", now - start < TEST_TIME_OUT);
			...
			assertFalse(status.getString("errorMessage"), BackupRestoreStatus.STATUS.FAILED.name().equals(status.getString("status")));
			// Are we done?
			if(BackupRestoreStatus.STATUS.COMPLETED.name().equals(statusString)){
				System.out.println("Backup Complete. Message: "+status.getString("progresssMessage"));
				System.out.println("Backup File: "+status.getString("backupUrl"));
				break;
			}else{
				...
				System.out.println("Backup progresss: "+percent+" % Message: "+message);
			}
			// Wait.
			Thread.sleep(1000);
		}
	...
	public void restoreFromBackup() throws Exception{
		...
		URL fileUrl = IT100BackupRestoration.class.getClassLoader().getResource(BACKUP_FILE_NAME);
		...
		JSONObject status = synapse.createEntity("/startRestoreDaemon", startUrl);
		assertNotNull(status);
		assertNotNull(status.getString("status"));
		assertFalse(status.getString("errorMessage"), BackupRestoreStatus.STATUS.FAILED.name().equals(status.getString("status")));
		assertTrue(BackupRestoreStatus.TYPE.RESTORE.name().equals(status.getString("type")));
		...
		while(true){
			long now = System.currentTimeMillis();
			assertTrue("Timed out waiting for the restore to complete", now - start < TEST_TIME_OUT);
			status = synapse.getEntity(backupUri);
			assertNotNull(status);
			String statusString = status.getString("status");
			assertNotNull(statusString);
			...
			// Are we done?
			if(BackupRestoreStatus.STATUS.COMPLETED.name().equals(statusString)){
				System.out.println("Restore Complete. Message: "+status.getString("progresssMessage"));
				break;
			}else{
				long current = status.getLong("progresssCurrent");
				long total = status.getLong("progresssTotal");
