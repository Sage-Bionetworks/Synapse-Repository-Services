...
if [ ${build_deploy} ]; then
	MVN_GOAL=deploy
	SETTINGS_XML="<settings><servers><server><id>github</id><username>${artifactory_username}</username><password>${artifactory_password}</password></server></servers></settings>"
fi
